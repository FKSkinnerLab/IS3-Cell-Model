...
Case1 = 'Case 9*';
ResultFiles = dir(['~/Desktop/SkinnerLab/Usages/Single Synapse Tests/' Case '/*.dat']);

for i=1:length(ResultFiles)
    load([['~/Desktop/SkinnerLab/Usages/Single Synapse Tests/' Case '/'] ResultFiles(i).name]); %eval(['load ' files1(i).name ' -dat']);
end
...
elseif strcmp(Case,'Case9StarOutput')
    ...
    text(295,8,'\leftarrow Tree 2A','FontSize',15) % Case 7 = 260; Case 8* = 260; Case 9* = 295
