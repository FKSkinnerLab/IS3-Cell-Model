...
#define noise _p[3]
#define count _p[4]
#define intervalVect (_p + 5)
#define event _p[105]
#define on _p[106]
#define ispike _p[107]
#define v _p[108]
#define _tsav _p[109]
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 110, 4);
 add_nrn_artcell(_mechtype, 3);
 add_nrn_has_net_event(_mechtype);
 pnt_receive[_mechtype] = _net_receive;
 pnt_receive_size[_mechtype] = 1;
 	hoc_register_var(hoc_scdoub, hoc_vdoub, hoc_intfunc);
 	ivoc_help("help ?1 pj /Users/alexgm/Desktop/SkinnerLab/Usages/Morphological Reduction/PurkReductionOnLine/x86_64/pj.mod\n");
...
static int  next_invl ( _threadargsproto_ ) {
   ...
   if ( ispike >= number ) {
     ispike = 0.0 ;
     number = normrand ( 15.0 , 5.0 ) ;
     if ( number <= 0.0 ) {
       number = - number + 1.0 ;
       }
     event = event + normrand ( 30.0 , 10.0 ) ;
     interval = intervalVect [ ((int) count ) ] ;
     count = count + 1.0 ;
     }
    return 0; }
...
static void initmodel(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 ...
 {
   count = 0.0 ;
   {int  _li ;for ( _li = 0 ; _li <= 99 ; _li ++ ) {
     intervalVect [ _li ] = normrand ( meani , 5.0 ) ;
     } }
   on = 0.0 ;
   ispike = 0.0 ;
   if ( noise < 0.0 ) {
