...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 15, 4);
 	...
 	ivoc_help("help ?1 CaT /Users/alexgm/Desktop/SkinnerLab/Usages/Morphological Reduction/PurkReductionOnLine/x86_64/CaT.mod\n");
...
static int _reset;
static char *modelname = "T calcium current";
...
static int  _f_rates ( _threadargsprotocomma_ double _lv ) {
 ...
 _lq10 = pow( 3.0 , ( ( celsius - 37.0 ) / 10.0 ) ) ;
   _ltinc = - dt * _lq10 ;
   _lalpha = 2.6 / ( 1.0 + exp ( ( _lv + 21.0 ) / ( - 8.0 ) ) ) ;
   _lbeta = 0.18 / ( 1.0 + exp ( ( _lv + 40.0 ) / 4.0 ) ) ;
   _lsum = _lalpha + _lbeta ;
   minf = _lalpha / _lsum ;
   mexp = 1.0 - exp ( _ltinc * _lsum ) ;
   _lalpha = 0.0025 / ( 1.0 + exp ( ( _lv + 40.0 ) / 8.0 ) ) ;
   _lbeta = 0.19 / ( 1.0 + exp ( ( _lv + 50.0 ) / ( - 10.0 ) ) ) ;
   _lsum = _lalpha + _lbeta ;
   hinf = _lalpha / _lsum ;
