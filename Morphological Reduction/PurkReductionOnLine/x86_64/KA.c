...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 13, 2);
 	...
 	ivoc_help("help ?1 KA /Users/alexgm/Desktop/SkinnerLab/Usages/Morphological Reduction/PurkReductionOnLine/x86_64/KA.mod\n");
...
static int _reset;
static char *modelname = "A current";
...
static int  _f_rates ( _threadargsprotocomma_ double _lv ) {
 ...
 _lq10 = pow( 3.0 , ( ( celsius - 37.0 ) / 10.0 ) ) ;
   _ltinc = - dt * _lq10 ;
   _lalpha = 1.4 / ( 1.0 + exp ( ( _lv + 27.0 ) / ( - 12.0 ) ) ) ;
   _lbeta = 0.49 / ( 1.0 + exp ( ( _lv + 30.0 ) / 4.0 ) ) ;
   _lsum = _lalpha + _lbeta ;
   minf = _lalpha / _lsum ;
   mexp = 1.0 - exp ( _ltinc * _lsum ) ;
   _lalpha = 0.0175 / ( 1.0 + exp ( ( _lv + 50.0 ) / 8.0 ) ) ;
   _lbeta = 1.3 / ( 1.0 + exp ( ( _lv + 13.0 ) / ( - 10.0 ) ) ) ;
   _lsum = _lalpha + _lbeta ;
   hinf = _lalpha / _lsum ;
