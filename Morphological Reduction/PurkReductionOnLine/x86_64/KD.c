...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 13, 2);
 	...
 	ivoc_help("help ?1 KD /Users/alexgm/Desktop/SkinnerLab/Usages/Morphological Reduction/PurkReductionOnLine/x86_64/KD.mod\n");
...
static int  _f_rates ( _threadargsprotocomma_ double _lv ) {
 ...
 _lq10 = pow( 3.0 , ( ( celsius - 37.0 ) / 10.0 ) ) ;
   _ltinc = - dt * _lq10 ;
   _lalpha = 8.5 / ( 1.0 + exp ( ( _lv + 17.0 ) / ( - 12.5 ) ) ) ;
   _lbeta = 35.0 / ( 1.0 + exp ( ( _lv + 99.0 ) / 14.5 ) ) ;
   _lsum = _lalpha + _lbeta ;
   minf = _lalpha / _lsum ;
   mexp = 1.0 - exp ( _ltinc * _lsum / 10.0 ) ;
   _lalpha = 0.0015 / ( 1.0 + exp ( ( _lv + 89.0 ) / 8.0 ) ) ;
   _lbeta = 0.0055 / ( 1.0 + exp ( ( _lv + 83.0 ) / ( - 8.0 ) ) ) ;
   _lsum = _lalpha + _lbeta ;
   hinf = _lalpha / _lsum ;
   hexp = 1.0 - exp ( _ltinc * _lsum * 1.6 ) ;
    return 0; }
...
static int _ode_count(int _type){ hoc_execerror("KD", "cannot be used with CVODE"); return 0;}
