...
Case = 'Case7';
% TopDirSim = '~/Desktop/SkinnerLab/Usages/NSG_Simulations/output/Case7Model/';
TopDirSim = '~/Desktop/SkinnerLab/Usages/Fast KDRF/Case 1/';
% TopDirSim = '~/Dropbox/MSC_THESIS/NEURON_files_mac/IS3_SimRuns/';
...
%     figure(Rank_To_Plot+2)
    ...
    xlabel('Time [ms]')
    ylabel('Voltage [mV]')
    title(['Model Rank Number = ' num2str(Rank_To_Plot)])
...
sorteddists = sortrows(sortrows(sortrows(sortrows(db_dists,2),3),4),5); % Sort rows for reshaping
Ndimdb = reshape(sorteddists(:,end),[NaTnumparams,NaPnumparams,Kdrfnumparams,Kanumparams]); % Reshapes distance vector into N-dimensional matrix
...
legend('off')
h = colorbar;
ylabel(h,'Normalized Cumulative Distance Metric','FontSize',18)
title('Clutter Based Dimensional Reordering Results','FontSize',18)
...
for i = 1:length(CIP_dists(1,:))
    ...
    legend('off')
    h = colorbar;
    ylabel(h,['Normalized ' num2str(CIP_str(i)) ' pA Distance Metric'],'FontSize',18)
    title('Clutter Based Dimensional Reordering Results','FontSize',18)
...
legend('off')
h = colorbar;
ylabel(h,'Frequency at Maximum Theta Peak (Hz)','FontSize',18)
title('Clutter Based Dimensional Reordering Results','FontSize',18)
...
legend('off')
h = colorbar;
ylabel(h,'Pre-CIP/CIP Average Potential Difference (mV)','FontSize',18)
title('Clutter Based Dimensional Reordering Results','FontSize',18)
