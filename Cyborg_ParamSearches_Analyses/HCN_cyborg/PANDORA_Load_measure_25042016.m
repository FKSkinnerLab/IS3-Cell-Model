...
Case = 'SDprox1_HCNS';
Cycle = '_N2';
% TopDirSim = '~/Desktop/SkinnerLab/Usages/NSG_Simulations/output/Case8StarPSIBAuRg/Cipres_Data/output/Case8StarModel/';
TopDirSim = ['~/Desktop/SkinnerLab/Usages/HCN_cyborg/NSG/Outputs/' Case Cycle '/Cipres_Data/output/' Case '/'];
...
PulseIni100msRest1SpikeRate = 17; % Number from column index in tests_db
PulseIni100msRest2SpikeRate = 19;
RecovSpikes = 201;
PulseSpikes = 152;
...
db_sim_p20.data(db_sim_m100.data(:,RecovSpikes)>0,:) = [];
db_sim_p50.data(db_sim_m100.data(:,RecovSpikes)>0,:) = [];
db_sim_p500.data(db_sim_m100.data(:,RecovSpikes)>0,:) = [];  
db_sim_m100.data(db_sim_m100.data(:,RecovSpikes)>0,:) = [];

db_sim_m100.data(db_sim_p20.data(:,RecovSpikes)>0,:) = [];
db_sim_p50.data(db_sim_p20.data(:,RecovSpikes)>0,:) = [];
db_sim_p500.data(db_sim_p20.data(:,RecovSpikes)>0,:) = [];
db_sim_p20.data(db_sim_p20.data(:,RecovSpikes)>0,:) = [];

db_sim_m100.data(db_sim_p50.data(:,RecovSpikes)>0,:) = [];
db_sim_p20.data(db_sim_p50.data(:,RecovSpikes)>0,:) = [];
db_sim_p500.data(db_sim_p50.data(:,RecovSpikes)>0,:) = [];
db_sim_p50.data(db_sim_p50.data(:,RecovSpikes)>0,:) = [];
...
ssd = sortrows(db_sim_m100.data(:,1:7),1);
...
for Rank_To_Plot = 1;
    ...
    f1 = [TopDirSim 'model_' num2str(ssd_params(1,1)) '_ModNum_'...
        ...
        '_Kdrf_' num2str(ssd_params(1,5)) '_KA_' num2str(ssd_params(1,6)) '_IH_' num2str(ssd_params(1,7)) '_DistFromSoma_-0.1_InjectionMagnitude.dat'];
    f2 = [TopDirSim 'model_' num2str(ssd_params(1,1)) '_ModNum_'...
        num2str(ssd_params(1,2)) '_NaT_' num2str(ssd_params(1,3)) '_NaP_' num2str(ssd_params(1,4)) ...
        '_Kdrf_' num2str(ssd_params(1,5)) '_KA_' num2str(ssd_params(1,6)) '_IH_' num2str(ssd_params(1,7)) '_DistFromSoma_0.02_InjectionMagnitude.dat'];
    f3 = [TopDirSim 'model_' num2str(ssd_params(1,1)) '_ModNum_'...
        num2str(ssd_params(1,2)) '_NaT_' num2str(ssd_params(1,3)) '_NaP_' num2str(ssd_params(1,4)) ...
        '_Kdrf_' num2str(ssd_params(1,5)) '_KA_' num2str(ssd_params(1,6)) '_IH_' num2str(ssd_params(1,7)) '_DistFromSoma_0.05_InjectionMagnitude.dat'];
    f4 = [TopDirSim 'model_' num2str(ssd_params(1,1)) '_ModNum_'...
        num2str(ssd_params(1,2)) '_NaT_' num2str(ssd_params(1,3)) '_NaP_' num2str(ssd_params(1,4)) ...
        '_Kdrf_' num2str(ssd_params(1,5)) '_KA_' num2str(ssd_params(1,6)) '_IH_' num2str(ssd_params(1,7)) '_DistFromSoma_0.5_InjectionMagnitude.dat'];
...
Ihnumparams = length(unique(m100db(:,6)));
DISTnumparams = length(unique(m100db(:,7)));
...
Ihmin = min(unique(m100db(:,6)));
DISTmin = min(unique(m100db(:,7)));
...
Ihmax = max(unique(m100db(:,6)));
DISTmax = max(unique(m100db(:,7)));
...
sorteddists = sortrows(sortrows(sortrows(sortrows(sortrows(sortrows(db_dists,2),3),4),5),6),7); % Sort rows for reshaping
Ndimdb = reshape(sorteddists(:,end),[NaTnumparams,NaPnumparams,Kdrfnumparams,Kanumparams,Ihnumparams,DISTnumparams]); % Reshapes distance vector into N-dimensional matrix
...
figure_dim_stack(round(Ndimdb*100),order,[0 100],cmap,...
    [],{['Na,t'],['Na,p'],['Kdrf'],['Ka'],['Ih'],['VGC Distance']});
legend('off')
h = colorbar;
...
for i = 1:length(CIP_dists(1,:))
    ...
    % Create N-Dimensional matrix of distance values
    sortedCIPdists = sortrows(sortrows(sortrows(sortrows(sortrows(sortrows(db_CIPdists,2),3),4),5),6),7); % Sort rows for reshaping
    NdimdbCIP = reshape(sortedCIPdists(:,end),[NaTnumparams,NaPnumparams,Kdrfnumparams,Kanumparams,Ihnumparams,DISTnumparams]); % Reshapes distance vector into N-dimensional matrix
    ...
    ['Ih (' num2str(Ihmin) ' to ' num2str(Ihmax) ' S/cm^2)'],...
    ['Distance (' num2str(DISTmin) ' to ' num2str(DISTmax) ' S/cm^2)']});
    legend('off')
    h = colorbar;
...
% Create N-Dimensional matrix of distance values
sortedFreqs = sortrows(sortrows(sortrows(sortrows(sortrows(sortrows(p20dbsorted,2),3),4),5),6),7); % Sort rows for reshaping
NdimdbFreq = reshape(sortedFreqs(:,FreqAtMaxThetaPeak),...
    [NaTnumparams,NaPnumparams,Kdrfnumparams,Kanumparams,Ihnumparams,DISTnumparams]); % Reshapes distance vector into N-dimensional matrix
...
% Create N-Dimensional matrix of distance values
sortedPotDiffs = sortrows(sortrows(sortrows(sortrows(sortrows(sortrows(p50dbsorted,2),3),4),5),6),7); % Sort rows for reshaping
...
figure(17)
hist3(ssd_params_dist_sorted(:,[2 5]),[NaTnumparams Kanumparams],'FaceAlpha',.65)
xlabel('NaT (S/cm^2)')
ylabel('Ka (S/cm^2)')
set(gcf,'renderer','opengl');
set(get(gca,'child'),'FaceColor','interp','CDataMode','auto');
...
figure(18)
hist3(ssd_params_dist_sorted(:,[2 7]),[NaTnumparams DISTnumparams],'FaceAlpha',.65)
xlabel('NaT (S/cm^2)')
ylabel('Distance (\mum)')
set(gcf,'renderer','opengl');
set(get(gca,'child'),'FaceColor','interp','CDataMode','auto');
...
figure(19)
hist3(ssd_params_dist_sorted(:,[2 6]),[NaTnumparams Ihnumparams],'FaceAlpha',.65)
xlabel('NaT (S/cm^2)')
ylabel('Ih (S/cm^2)')
set(gcf,'renderer','opengl');
set(get(gca,'child'),'FaceColor','interp','CDataMode','auto');
...
figure(21)
hist3(ssd_params_dist_sorted(:,[3 5]),[NaPnumparams Kanumparams],'FaceAlpha',.65)
xlabel('NaP (S/cm^2)')
ylabel('Ka (S/cm^2)')
set(gcf,'renderer','opengl');
set(get(gca,'child'),'FaceColor','interp','CDataMode','auto');
...
figure(22)
hist3(ssd_params_dist_sorted(:,[3 7]),[NaPnumparams DISTnumparams],'FaceAlpha',.65)
xlabel('NaP (S/cm^2)')
ylabel('Distance (\mum)')
set(gcf,'renderer','opengl');
set(get(gca,'child'),'FaceColor','interp','CDataMode','auto');
...
figure(23)
hist3(ssd_params_dist_sorted(:,[3 6]),[NaPnumparams Ihnumparams],'FaceAlpha',.65)
xlabel('NaP (S/cm^2)')
ylabel('Ih (S/cm^2)')
set(gcf,'renderer','opengl');
set(get(gca,'child'),'FaceColor','interp','CDataMode','auto');
...
% Kdrf
figure(24)
hist3(ssd_params_dist_sorted(:,[4 5]),[Kdrfnumparams Kanumparams],'FaceAlpha',.65)
xlabel('Kdrf (S/cm^2)')
ylabel('Ka (S/cm^2)')
set(gcf,'renderer','opengl');
set(get(gca,'child'),'FaceColor','interp','CDataMode','auto');
...
figure(25)
hist3(ssd_params_dist_sorted(:,[4 7]),[Kdrfnumparams DISTnumparams],'FaceAlpha',.65)
xlabel('Kdrf (S/cm^2)')
ylabel('Distance (\mum)')
set(gcf,'renderer','opengl');
set(get(gca,'child'),'FaceColor','interp','CDataMode','auto');
...
figure(26)
hist3(ssd_params_dist_sorted(:,[4 6]),[Kdrfnumparams Ihnumparams],'FaceAlpha',.65)
xlabel('Kdrf (S/cm^2)')
ylabel('Ih (S/cm^2)')
set(gcf,'renderer','opengl');
set(get(gca,'child'),'FaceColor','interp','CDataMode','auto');
...
% Ka
figure(27)
hist3(ssd_params_dist_sorted(:,[5 7]),[Kanumparams DISTnumparams],'FaceAlpha',.65)
xlabel('Ka (S/cm^2)')
ylabel('Distance (\mum)')
set(gcf,'renderer','opengl');
set(get(gca,'child'),'FaceColor','interp','CDataMode','auto');
...
figure(28)
hist3(ssd_params_dist_sorted(:,[5 6]),[Kanumparams Ihnumparams],'FaceAlpha',.65)
xlabel('Ka (S/cm^2)')
ylabel('Ih (S/cm^2)')
set(gcf,'renderer','opengl');
set(get(gca,'child'),'FaceColor','interp','CDataMode','auto');
...
PulseIniSpontPotAvgDiff = 25;
PulsePotMin = 31;
PulsePotMinTime = 32;
PulsePotSag = 33;
PulsePotTau = 34;
FreqAtMaxThetaPeak = 8;
ThetaPower = 54;
PulseFirstSpikeTime = 14;
PulseSFA = 35;
PulseSpikeRate = 38;
PulseSpikeRateISI = 39;
PulseSpikeAmplitudeMean = 104;
PulseSpikeHalfWidthMean = 122;
PulseSpikeInitVmMean = 131;
PulseSpikeMaxAHPMean = 134;
PulseSpikes = 152;
PulseIni100msSpikeRate = 23;
PulseIni100msRestIniSpontPotAvgDiff = 21;
...
display('Model #      G_NaT        N_NaP         G_Kdrf        G_Ka          G_Ih  DistanceFromSoma  EuclideanDistance')
