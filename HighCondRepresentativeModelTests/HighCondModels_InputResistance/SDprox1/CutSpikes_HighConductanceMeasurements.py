...
for x in range(0,len(ParamNums[0])):
	...
	for y in range(1,11):
		...
		RiIndex[y-1] = results[2]
	MeanRi[x] = numpy.mean(RiIndex) # Divide by number of iterations to find mean
	StdRi[x] = numpy.std(RiIndex)
	RiIndex = numpy.zeros((10,), dtype=numpy.float64) # Re-initialize SpikeRateIndex for next iteration
