...
tstop = h.tstop
dt = h.dt
def getMeasures(inhsyn,excsyn,inhspikes,excspikes,examplenum,repnum,inhtheta,exctheta,EXCSLM,EXCSR,OLM,NGF,IS2,BIS,IS1,holding):	
	rep = int(repnum)
	example = int(examplenum)
	h.randomize_syns(example,rep) # Randomizes synapse location with a different seed on each repetition
	h.f(int(inhsyn),int(excsyn),int(inhspikes),int(excspikes),0,example,rep,1,inhtheta,exctheta,EXCSLM,EXCSR,OLM,NGF,IS2,BIS,IS1,int(holding)) # Runs Simulation
...
for x in range(0,len(ParamNums[0])):
	...
	results = getMeasures(numinh,0,inhspikes,0,1,1000,numinhtheta,0,0,0,1,1,1,1,1,0) # i.e. same random seed values for all representative scenarios
