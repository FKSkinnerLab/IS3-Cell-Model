...
for x in range(0,len(ParamNums[0])):
	...
	for y in range(0,2):
		...
		if y == 0:
			...
			conductancevec1 = (results[1])/((holding_potentials[y]-holding_potentials[y+1])*0.001) # Results in Nanosiemens
		elif y == 1:
			timevec2 = results[0]
			currentvec2 = results[1]*1000
			conductancevec2 = (results[1])/((holding_potentials[y]-holding_potentials[y-1])*0.001)
	EIratio1 = numpy.mean(numpy.divide(conductancevec2,conductancevec1))
	EIratio11 = numpy.divide(numpy.mean(conductancevec2),numpy.mean(conductancevec1))
	...
	timevec1 = results[0]
	currentvec1 = results[1]*1000
	conductancevec1 = (results[1])/((-60-(-70))*0.001)
	...
	timevec2 = results[0]
	currentvec2 = results[1]*1000
	conductancevec2 = (results[1])/((-60-(0))*0.001)
	...
	timevec1 = results[0]
	currentvec1 = results[1]*1000
	conductancevec1 = (results[1])/((0-(-70))*0.001)
	...
	EIratio2 = numpy.mean(numpy.divide(conductancevec2,conductancevec1))
	EIratio22 = numpy.divide(numpy.mean(conductancevec2),numpy.mean(conductancevec1))
	f, axarr = matplotlib.pyplot.subplots(1)
	axarr.plot(timevec1,currentvec1,colors[0])
	axarr.plot(timevec2,currentvec2,colors[1])
	axarr.set_xlabel('Time (ms)')
	...
	pyplot.clf()
	pyplot.close()
	print 'Divide First - Not Removed E/I Ratio: ' + numpy.array2string(EIratio1) + '; Removed E/I Ratio: ' + numpy.array2string(EIratio2)
	print 'Mean First - Not Removed E/I Ratio: ' + numpy.array2string(EIratio11) + '; Removed E/I Ratio: ' + numpy.array2string(EIratio22)
elapsed = time.time() - t
print elapsed
