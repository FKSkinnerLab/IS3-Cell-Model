...
tstop = 10 # seconds
font_size = 6
numpy.save('NPYfiles/' + Case + '_MeanSpikeRatesHCModels.npy',MeanSpikeRates)
numpy.save('NPYfiles/' + Case + '_StdSpikeRatesHCModels.npy',StdSpikeRates)

f, axarr = matplotlib.pyplot.subplots(1)
ind = numpy.arange(16)
width = 0.4
axarr.bar(ind+width, MeanSpikeRates, width, color='k',yerr=StdSpikeRates,ecolor='r')
...
axarr.set_ylabel('Mean Spike Rate (Hz)')
axarr.set_xlim(-0.2,16+width)
pyplot.savefig('PLOTfiles/' + Case + '_MeanSpikeRatesHCModels_' + '.pdf', bbox_inches='tight')
pyplot.savefig('PLOTfiles/' + Case + '_MeanSpikeRatesHCModels_' + '.png', bbox_inches='tight')
pyplot.gcf().clear()
pyplot.cla()
pyplot.clf()
pyplot.close()
