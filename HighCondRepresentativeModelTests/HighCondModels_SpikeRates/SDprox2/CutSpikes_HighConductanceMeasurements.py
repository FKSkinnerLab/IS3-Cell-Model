...
def getMeasures(inhsyn,excsyn,inhspikes,excspikes,examplenum,repnum):	
	...
	outputresults = [rep,HC_Metric,SpikeCount]
	return outputresults

# Set up parallel context
CumulHCs = numpy.zeros((16,), dtype=numpy.float64)
MeanSpikeRates = numpy.zeros((16,), dtype=numpy.float64)
StdSpikeRates = numpy.zeros((16,), dtype=numpy.float64)
SpikeRateIndex = numpy.zeros((10,), dtype=numpy.float64)
for x in range(0,len(ParamNums[0])):
	...
	for y in range(1,11):
		results = getMeasures(numinh,numexc,inhspikes,excspikes,1,y) # i.e. same random seed values for all representative scenarios
		print 'HC1 = ' + str(results[1]) + ', for example #' + str(x) + ' on rep #' + str(y)
		if results[1] == 3:
			CumulHCs[x] = CumulHCs[x] + 1
		SpikeRateIndex[y-1] = results[2]/(tstop/1000)
	MeanSpikeRates[x] = numpy.mean(SpikeRateIndex) # Divide by number of iterations to find mean
	StdSpikeRates[x] = numpy.std(SpikeRateIndex)
	SpikeRateIndex = numpy.zeros((10,), dtype=numpy.float64) # Re-initialize SpikeRateIndex for next iteration
