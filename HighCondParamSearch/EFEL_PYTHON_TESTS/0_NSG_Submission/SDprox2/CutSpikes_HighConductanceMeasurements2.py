    ...
    for i_NumExc in range(0,len(NumExc)):
        for i_InhSpikes in range(0,len(InhSpikes)):
			...
			NumInhCommonVec[Vec_count] = NumInhCommon[i_NumInhCommon]
                        Vec_count = Vec_count + 1
Vec_count = Vec_count - 1 # Correct count
...
def getMeasures(trace_index):
	i_NumInh = NumInhVec[trace_index]
	i_NumExc = NumExcVec[trace_index]
	i_InhSpikes = InhSpikesVec[trace_index]
	i_ExcSRSpikes = ExcSRSpikesVec[trace_index]
	i_NumExcCommon = NumExcCommonVec[trace_index]
	i_NumInhCommon = NumInhCommonVec[trace_index]
	...
	return StdVolt[trace_index]
	return MeanVolt[trace_index]
	return NumSpikes[trace_index]
	return MeanAPamp[trace_index]
	return ISICV[trace_index]
	return NumInhVec[trace_index]
	return NumExcVec[trace_index]
	return InhSpikesVec[trace_index]
	return ExcSRSpikesVec[trace_index]
	return ExcSLMSpikesVec[trace_index]
	return NumExcCommonVec[trace_index]
	return NumInhCommonVec[trace_index]
...
pool = Pool(NUM_PROCS)
pool.map(getMeasures,range(0,Vec_count))
pool.close()
pool.join()
