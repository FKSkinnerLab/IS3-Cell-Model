		...
		for y in range(0,4):
			...
			if thetaSynMultiplier[y]>0:
				...
				for l in range(0,int(h.inhthetacount)):
					randind = int(h.OLMrandSLMtheta.x[l])
					if randind == -1: randind = 0
					ThetaVec = numpy.array([],dtype=numpy.float)
					for q in range(0,len(h.ThetaSLMOLMprespiketrains[randind])): ThetaVec = numpy.append(ThetaVec,h.ThetaSLMOLMprespiketrains[randind].x[q])
					ThetaVec = numpy.array(ThetaVec,dtype=numpy.float)
					axarr2.vlines(ThetaVec,l+j+k+2.75,l+j+k+3.25,'m')
				for m in range(0,int(h.inhthetacount)):
					randind = int(h.IS2randSLMtheta.x[m])
					if randind == -1: randind = 0
					ThetaVec = numpy.array([],dtype=numpy.float)
					for q in range(0,len(h.ThetaSLMIS2prespiketrains[randind])): ThetaVec = numpy.append(ThetaVec,h.ThetaSLMIS2prespiketrains[randind].x[q])
					ThetaVec = numpy.array(ThetaVec,dtype=numpy.float)
					axarr2.vlines(ThetaVec,m+l+j+k+3.75,m+l+j+k+4.25,'y')
				for n2 in range(0,int(h.inhthetacount)):
					randind = int(h.NGFrandSLMtheta.x[n2])
					if randind == -1: randind = 0
					ThetaVec = numpy.array([],dtype=numpy.float)
					for q in range(0,len(h.ThetaSLMNGFprespiketrains[randind])): ThetaVec = numpy.append(ThetaVec,h.ThetaSLMNGFprespiketrains[randind].x[q])
					ThetaVec = numpy.array(ThetaVec,dtype=numpy.float)
					axarr2.vlines(ThetaVec,n2+m+l+j+k+4.75,n2+m+l+j+k+5.25,'b')
				for o in range(0,int(h.inhthetacount)):
					randind = int(h.BISrandSRtheta.x[o])
					if randind == -1: randind = 0
					ThetaVec = numpy.array([],dtype=numpy.float)
					for q in range(0,len(h.ThetaSRBISprespiketrains[randind])): ThetaVec = numpy.append(ThetaVec,h.ThetaSRBISprespiketrains[randind].x[q])
					ThetaVec = numpy.array(ThetaVec,dtype=numpy.float)
					axarr2.vlines(ThetaVec,o+n2+m+l+j+k+5.75,o+n2+m+l+j+k+6.25,'k')
				for p in range(0,int(h.inhthetacount)):
					randind = int(h.IS1randSRtheta.x[p])
					if randind == -1: randind = 0
					ThetaVec = numpy.array([],dtype=numpy.float)
					for q in range(0,len(h.ThetaSRIS1prespiketrains[randind])): ThetaVec = numpy.append(ThetaVec,h.ThetaSRIS1prespiketrains[randind].x[q])
					ThetaVec = numpy.array(ThetaVec,dtype=numpy.float)
					axarr2.vlines(ThetaVec,p+o+n2+m+l+j+k+6.75,p+o+n2+m+l+j+k+7.25,color='orange')
				axarr2.set_xlim(8000,10000)
				axarr2.set_ylim(0,p+o+n2+m+l+j+k+8)
				axarr2.set_xlabel('Time (ms)',fontsize=font_size-4)
				axarr2.set_ylabel('CA3                   EC3                  OLM   IS2   NGF   BIS   IS1',fontsize=font_size-4)
