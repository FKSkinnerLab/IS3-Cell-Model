...
ExcDirectory = '~/Desktop/SkinnerLab/Usages/LayerSpecificInputTests/PlotOptimizedPSCs_N2/Output/modelE_*_DendriteNumber.dat';
InhDirectory = '~/Desktop/SkinnerLab/Usages/LayerSpecificInputTests/PlotOptimizedPSCs_N2/Output/modelI_*_DendriteNumber.dat';
GenDirectory = '~/Desktop/SkinnerLab/Usages/LayerSpecificInputTests/PlotOptimizedPSCs_N2/Output/';
font_size = 20;
font_weight = 'normal';
font_angle = 'normal';
...
SLMEPSC_exp = load('~/Desktop/SkinnerLab/Usages/LayerSpecificInputTests/PassiveOptimization_N1/PPDatFile.dat');
SLMEPSC_exp(:,2) = SLMEPSC_exp(:,2).*1000;
RADEPSC_exp = load('~/Desktop/SkinnerLab/Usages/LayerSpecificInputTests/PassiveOptimization_N1/SCDatFile.dat');
RADEPSC_exp(:,2) = RADEPSC_exp(:,2).*1000;
IPSC_exp = load('~/Desktop/SkinnerLab/Usages/LayerSpecificInputTests/PassiveOptimization_N1/ISPCDatFile.dat');
...
%% Plot specific PSCs
% i = 30;
for i = 1:length(data_Exc(1,:))
    fig4 = figure(4);
    
    % IPSC
    plot(tvecInh,data_Inh(:,i),'Color','r', 'LineWidth', 10)
    hold on
    % EPSC
    plot(tvecExc+4.5,data_Exc(:,i),'Color','b', 'LineWidth', 10)
    
    if i == 163
        plot([8 12],[-30 -30],'Color','k', 'LineWidth', 10)
        text(8.8,-36,'4 ms','FontSize',font_size+25)
        plot([12 12],[-15 -30],'Color','k', 'LineWidth', 10)
        text(12.5,-22.5,'15 pA','FontSize',font_size+25)
    end
    ...
    saveas(fig4,['AllPSCPlots/Dist' num2str(dist_vec(i)) '_Sect' num2str(dendnum_vec(i)) '.pdf'])
    saveas(fig4,['AllPSCPlots/Dist' num2str(dist_vec(i)) '_Sect' num2str(dendnum_vec(i)) '.png'])
    close all
end