...
directory1 = '~/Desktop/SkinnerLab/Usages/LayerSpecificInputTests_N2/Case7Output/';
directory2 = '~/Desktop/SkinnerLab/Usages/LayerSpecificInputTests_N2/Case8Output/';
directory3 = '~/Desktop/SkinnerLab/Usages/LayerSpecificInputTests_N2/Case8StarOutput/';
directory4 = '~/Desktop/SkinnerLab/Usages/LayerSpecificInputTests_N2/Case9StarOutput/';
font_size = 20;
font_weight = 'normal';
font_angle = 'normal';
...
dist_vec1((dist_vec1(:,1) == 0),:) = []; % Try commenting this out to see what happens
...
for i = 1:length(dist_vec1)-1
    ...
    if Dendsec_vec1(i+1) ~= Dendsec_vec1(i) || i+2 > length(Dendsec_vec1) % i.e. if next data point is on a new section
        ...
        t = t(t<l); % all t index points must be earlier (smaller) than the next branching point index
        if isempty(t) % if no smaller distances are available connect to first data point
            t = l; % If no previous data points, than just replot the point after the branching point (since origin index is continuous from the proximal dendrites and therefore cutoff in this graph)
        end
        t = t(end); % Choose most recent distance that is smaller than the next branching point
